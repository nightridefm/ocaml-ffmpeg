...
CAMLprim value ocaml_avfilter_init(value unit) {
  ...
  /* An AVFilterGraph handle is an 8-byte pointer, but once filters are
     ...
     converter). Liquidsoap builds one of these PER DECODED FILE: the
     ffmpeg internal video decoder creates its Fps converter per stream
     (`mk_video_decoder`, decoder/ffmpeg_internal_decoder.ml), so a
     playlist rotating short clips drops a configured graph on every
     rotation for the GC to finalize. Accounting each handle as ~nothing
     (the old `1, 0`) let the tiny OCaml major heap go a long time between
     collections, so those finalizers fired late and the
